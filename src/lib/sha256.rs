...
pub fn hash(msg: &[u8]) -> Option<()> {
    ...
    // add message length
    copy_len_to_buf(&mut temp_block_buf, msg_len);
    /* padding message end */

    println!("Buffer u8:");
    for i in 0..temp_block_buf.len() {
        print!("{val:#04x} ", val = temp_block_buf[i]);
    }
    println!("");

    for i in (0..temp_block_buf.len()).step_by(BUFFER_SIZE_U8) {
        // copy into active block buffer
        copy_buf_u8_to_u32(&mut temp_block_buf, &mut cur_block_buf, i);

        println!("{i}: Buffer u32:");
